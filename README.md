# FRC 9012 Team Waffles REBUILT Season

* Pneumatic Hub (CAN ID 2)
* MaxSwerve using NEO Vortex motors and NEO 550 motors (CAN IDs 3,4,5,6,7,8,9,10)
* 2 Elevator NEO motors (CAN IDs 11, 12)
* 2 Intake NEO 550 motors (CAN IDs 13, 14)
* 2 Launchers NEO Vortex (CAN IDs 15, 16)
* 2 Transfer NEO550 motors (CAN IDs 17, 18)
* 
* Pneumatic Cylinders 0,1 for Intake pivot
* Pneumatic Cylinders 2,3 for Hopper

## MaxSwerve Prerequisites

* SPARK MAX Firmware v26.1.0
* REVLib v2026.0.0

## MaxSwerve Configs & Constants

* Drivetrain Gear Max Speed: 7.22 m/s
* TrackWidth: 22.5 inches
* DrivingMotor Pinion Teeth: 16T
* SpurGear Teeth: 20T
* NEO Vortex FreeSpeed RPM: 6784
* Robot Raw Weight: 115 lbs
* Blue Bumper Weight: 12.38 lbs; Red Bumper Weight: 12.30 lbs; Battery Weight: 13.6 lbs
* Total Weight: 141 lbs

Update using `Constants.java` file and adjust Pathplanner configurations accordingly.